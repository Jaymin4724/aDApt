...
export default function LostnFound() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          `${baseURL}/lost-and-found/categories`,
          {
    ...
    const fetchItems = async () => {
      try {
        const response = await axios.get(`${baseURL}/lost-and-found/items`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDelete = async (itemId) => {
    try {
      await axios.delete(`${baseURL}/lost-and-found/item/${itemId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
        ...
        <div className="w-full md:w-3/4 lg:w-5/6 bg-white p-4 overflow-auto shadow-md rounded-lg">
          ...
          <div className="my-5 flex gap-2">
            ...
            <button
              ...
              onClick={() => navigate("/lostnfound/add")}
            >
              <Add />
            </button>
          </div>
